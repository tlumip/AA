...
class SSessionJdbcHelper {
    ...
    void flush(SRecordInstance instance) {
    	...
    	if (!instance.isDirty()) return; // flushing twice not a user bug.
    	try {
    		...
    		if (!instance.isDeleted()) {
    			for (SFieldScalar sfm : meta.getAllScalarFields()) {
					boolean upd = false;
					if ( instance.isNewRow() && sfm.isPrimary() ) {
						if ( sfm.getGenerator() == null ) upd = true;
						else if ( ((SGenerator) sfm.getGenerator()).includeGeneratedKeysInInsertValues(session) ) 
							upd = true;
						else
							hasFields = true;
					}
