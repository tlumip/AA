 ...
 * as required. The driver type is infered from the jdbc connection's meta data.
 * <p>
 *  (Profiling suggests that memoising these generaters could produce a 5-10%
 ...
 * varying(n), date, float(p), real, double precission, smallint, int | integer,
...
//   registers itself when the class is loaded and can make instances of the parent SDriver.
	...
	static {
		SDriver[] ds = new SDriver[] { new SDriverHSQL(), new SDriverH2(),
				new SDriverPostgres(), new SDriverMySQL(), new SDriverOracle(),
				new SDriverMSSQL(), new SDriverMSSqlServer(), new SDriverDB2_400(),
				new SDriverInformix(), new SDriverInterbase(),
				new SDriverFirebird(), new SDriverSapDB(), new SDriverSybase(),
				new SDriverDerby() };
	...
	static SDriver newSDriver(java.sql.Connection con) {
		...
		SLog.getSessionlessLogger().warn(
    ...
    protected void appendQuotedIdentifier(String ident, StringBuffer buf, char quote) {
        ...
        for (int cx=0; cx<ident.length(); cx++) {
            ...
            buf.append(ch);
            if (ch == quote) buf.append(quote); // double up any double quotes
        }
        buf.append(quote);
	}
	...
	protected String wholeColumnSQL(SFieldScalar fld) {
        ...
        if ( fld.sqlDataTypeOverride != null ) 
            ...
            sql.append(columnTypeSQL(fld, fld.defaultSqlDataType()));
		addNull(sql, fld);
        if (fld.getGeneratorMode() == SGeneratorMode.SINSERT)
            sql.append(addInsertGenerator(fld));
...
//				if (fld.isPrimary()) {
	...
	private void makeForeignKeySQL(SRecordMeta<?> meta, int fx,
		...
		if (spare < 0)
			throw new SException.Error("Table name '" + tname
					+ "' is longer than " + maxIdentNameLength() + " - "
					+ (fxStr.length() + 1) + " chars as permitted by "
					+ driverName() + " to allow for _nn constraint name.");
		String fkeyName = tname + "_" + fxStr; // Ensure unique
		String fname = fldRef.getFieldName();
		if (spare > 1)
			fkeyName += "_"; // >1 so no trailing _
	 ...
	 * limits.
	 */
	protected <RI extends SRecordInstance> String selectSQL(SFieldScalar[] select, SRecordMeta<RI> from,
			SFieldScalar[] where, String orderBy, boolean forUpdate) {
		...
		return selectSQL(selectBuf.toString(),
				         fromSQL(new SQueryTable<RI>(from.getTableName(), from, select, 1)),
				         null, // join
				         wherestr.toString(),
				         null, // group by
				         orderBy,
				         forUpdate, Integer.MAX_VALUE, 0);
	}
	 ...
	 * @return LIMIT + OFFSET string statment
	 */
	...
	protected String joinSQL(SQueryTable<?> table) {
        ...
        res.append(" ");
		// Only append the alias if it is different from the table name
		...
		if (table.getRawOnClause() != null)
	        ...
	        boolean first = true;
	        // Test if this is a one-to-many join. If the refField has the same table on both side
	        // (autojoin, eg. Employee.MANAGER), defaults to false, which is more natural in sorm.
	        // ## Notice for autojoin we cannot -select manager join employee-, it will always be employee join manager
	        boolean oneToManyJoin = (table.getFieldReference().getRecordMeta() == table.getRecordMeta()
	        		                     && table.getFieldReference().getReferencedRecordMeta() != table.getRecordMeta());
			for (SFieldScalar fkey : table.getFieldReference().getForeignKeyMetas()) {
				...
				else { // other way, more natural in simpleorm. Also is thedefault order when autojoining tables
					   // eg. emp --> manager rather than manager --> employee (not possible in simpleorm)
	 ...
	 */
	protected String updateSQL(ArrayList<SFieldScalar> updates, SRecordMeta<?> from,
			ArrayList<SFieldScalar> where, SRecordInstance instance, Object[] keyMetaValues) {
		StringBuffer ret = new StringBuffer(200);
		ret.append("UPDATE ");
		appendTableName(from, ret);  ret.append(" SET ");
	...
	protected String insertSQL(ArrayList<SFieldScalar> updates, SRecordMeta<?> from) {
		...
		appendTableName(from, ret); ret.append(" (");
		for (int sx = 0; sx < updates.size(); sx++) {
			if (sx > 0)
				ret.append(", ");
			SFieldScalar sfld = updates.get(sx);
	 ...
	 */
	protected String deleteSQL(SRecordMeta<?> from, ArrayList<SFieldScalar> where,
			SRecordInstance instance, Object[] keyMetaValues) {
		StringBuffer ret = new StringBuffer(200);
	 ...
	 * the instance values so that it can use the IS NULL test (for optimisitic
	 * locking).
	 */
	protected void whereSQL(StringBuffer ret, ArrayList<SFieldScalar> where,
			SRecordInstance instance, Object[] keyMetaValues) {
		ret.append(" WHERE ");
	...
	protected void appendField(SQueryTable<?> tbl, SFieldScalar sclField, StringBuffer buf) {
		...
		if ( ! aliased)
			appendTableName(tbl.getRecordMeta(), buf);
		buf.append(".");
	 ...
	 * BULK Simpleorm will get the whole resultset and skip de first row. Worst case scenario :(
	 * 
	 * Defaults to BULK
	 */
	...
	protected long generateKeySelectMax(SRecordMeta<?> rec, SFieldScalar keyFld) {
		...
		if (next == null)
			...
			db = ((Number) next).longValue();
		else
			db = Long.parseLong(next.toString());
    ...
    protected boolean includeGeneratedKeysInInsertValues() {return true;}
	 ...
	 */
	public enum OffsetStrategy {JDBC, QUERY, SCAN}
    ...
    public SLog getLogger() {return getSession().getLogger();}
