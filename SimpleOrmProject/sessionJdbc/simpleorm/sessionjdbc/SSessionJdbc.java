...
public class SSessionJdbc extends SSessionI {
	...
	public static SSessionJdbc open(Connection con, String connectionName, SDriver driver, String schema) {
		try {
			setSchema(schema, con);
		} catch (SQLException e) {
			throw new SException.Jdbc("Setting schema to "+schema, e);
		}
		SSessionJdbc ses = new SSessionJdbc();
		ses.innerOpen(con, connectionName, driver);
		return ses;
	...
	public static SSessionJdbc open(DataSource source, String connectionName, SDriver driver, String schema) {
		Connection con = null;
			try {
	 ...
	 * @throws SQLException
	 */
	private static void setSchema(String schema, Connection con)
			throws SQLException {
		if (schema!=null) {
			if (schema.equals("")) {
				Logger.getLogger(SSessionJdbc.class).info("schema is not set, using default (dbo or public)");
			} else if ((schema.equalsIgnoreCase("public") || schema.equalsIgnoreCase("dbo"))) {
				Logger.getLogger(SSessionJdbc.class).info("schema is "+schema);
			} else {
	...
	public static SSessionJdbc open(DataSource source, String connectionName, String schema) {
		return open(source, connectionName,  null, schema);
	}
...
//		if (getLogger().enableDebug()) {
		try {
			...
			getLogger().connections("Opened Connection " + this + " SimpleORM " + SUte.simpleormVersion() + " jdk " + jvsn 
					+ " Driver " + con.getMetaData().getDriverName());
...
//		}
	...
	//CheckWithJohn test here..
	// deleted, using begin(SDataset) now to avoid long transactions
//	public void attachDataSet(SDataSet dataSet){
//		if (this.dataSet != null)
//			throw new SException.InternalError("Non empty existing session dataset/update list.");
//        if (dataSet.getSession() != null)
//            throw new SException.Error("DataSet already has a session " + dataSet + dataSet.getSession());
//        dataSet.bindSession(this); 
