...
public abstract class SFieldMeta // implements Serializable { // See SRecordInstance.getMeta()
{
	...
	public enum SFieldProperty {PROMPT, TOOLTIP};
...
//	}
	 ...
	 */
     ...
     ///////////// Known properties Accessors

     /**
      * Shortcut to put a user prompt in UserProperties with key SFieldProperty.PROMPT
      * Can be used to set a default field label in UI.
      * TODO generate DDL COMMENT ON statements
      */
     public  <T extends SFieldMeta> T setPrompt(String prompt) {
    	 return putUserProperty(SFieldProperty.PROMPT, prompt);
     }
     
     /**
      * Shortcut to get user prompt from UserProperties (key SFieldProperty.PROMPT)
      */
     public  String getPrompt() {
    	 // we don't check that value is actually a String. It should be if setPrompt was used,
    	 // else it can be considered a programmer's bug...
    	 return getUserProperty(SFieldProperty.PROMPT);
     }
     
     /**
      * Shortcut to put a tooltip in UserProperties with key SFieldProperty.TOOLTIP
      * Can be used to set a default tooltip in UI.
      * TODO generate DDL COMMENT ON statements
      */
     public  <T extends SFieldMeta> T setTooltip(String tooltip) {
    	 return putUserProperty(SFieldProperty.TOOLTIP, tooltip);
     }
     
     /**
      * Shortcut to get tooltip from UserProperties (key SFieldProperty.TOOLTIP)
      */
     public  String getTooltip() {
    	 // we don't check that value is actually a String. It should be if setTooltip was used,
    	 // else it can be considered a programmer's bug...
    	 return getUserProperty(SFieldProperty.TOOLTIP);
     }
}
